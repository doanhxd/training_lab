...
class DemoOnlyRsiquiMt5Runner:
    ...
    def start(self) -> bool:
        ...
        for symbol in self._symbols_required_for_start():
            ...
            if self.mt5.symbol_info(symbol) is None:
                ...
                return False
        self._started = True
        self.last_status = f"ready: DEMO {self._active_symbol()} {self.config.timeframe} closed-bar RSIQUI V3 NEG ({self.config.preset})"
        return True
    ...
    def poll_once(self) -> bool:
        ...
        if side is None or bar_time is None:
            self.last_status = "no closed-bar RSIQUI V3 NEG signal"
            return False
