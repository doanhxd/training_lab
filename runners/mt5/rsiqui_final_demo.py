...
class DemoOnlyRsiquiMt5Runner:
    ...
    def start(self) -> bool:
        ...
        for symbol in self._symbols_required_for_start():
            ...
            if self.mt5.symbol_info(symbol) is None:
                ...
                return False
        self._started = True
        self.last_status = f"ready: DEMO {self._active_symbol()} {self.config.timeframe} pre-close RSIQUI V3 FINAL ({self.config.preset})"
        return True
    ...
    def poll_once(self, now_utc: datetime | None = None) -> bool:
        ...
        if side is None or evaluated_bar is None:
            self.last_status = "no pre-close RSIQUI V3 FINAL signal" if not self.last_status.startswith("blocked:") else self.last_status
            return False
