...
def load_demo_config(path: str | Path) -> Mt5DemoConfig:
    ...
    if payload.get("strategy") != "rsiqui-v3-final":
        raise ValueError("runner accepts only strategy rsiqui-v3-final")
...
class DemoOnlyRsiquiMt5Runner:
    ...
    def start(self) -> bool:
        ...
        if self.config.equity_risk_cap_pct is None:
            self._effective_risk_usd = self.config.risk_usd
        else:
            self._effective_risk_usd = min(self.config.risk_usd, float(account.equity) * self.config.equity_risk_cap_pct)
        ...
        for symbol in self._symbols_required_for_start():
            ...
            if self.mt5.symbol_info(symbol) is None:
                ...
                return False
        self._started = True
        self.last_status = f"ready: DEMO {self._active_symbol()} {self.config.timeframe} closed-bar RSIQUI V3 FINAL ({self.config.preset})"
        return True
    ...
    def poll_once(self) -> bool:
        ...
        if side is None or bar_time is None:
            self.last_status = "no closed-bar RSIQUI V3 FINAL signal"
            return False
