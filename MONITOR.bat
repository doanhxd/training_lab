...
if not exist "%PROJECT_ROOT%configs\strategies\rsiqui\final_m5.json" set "PROJECT_ROOT=%USERPROFILE%\Desktop\trading_lab\"

set "PYTHONW=%PROJECT_ROOT%.venv\Scripts\pythonw.exe"
if not exist "%PYTHONW%" (
    echo Khong tim thay project venv: %PYTHONW%
    echo Hay cai dependencies cua trading_lab truoc.
    exit /b 1
)
