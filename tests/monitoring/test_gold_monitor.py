...
class GoldMonitorTests(TestCase):
    ...
    def test_requested_home_and_dialog_visual_contracts_are_present(self):
        ...
        self.assertIn('legend.append(("ALL — TOÀN BỘ LỊCH SỬ" if self._equity_aggregate_only else "ALL", Palette.TEXT))', source)
        ...
        self.assertIn('def _refresh_full_equity_curve(self)', source)
        self.assertIn('self._equity_aggregate_only = True', source)
        self.assertIn('if self._equity_aggregate_only:', source)
        self.assertIn('ALL — TOÀN BỘ LỊCH SỬ', source)
