...
RAW_PATH = ROOT / "data" / "raw" / "mt5_exports" / "XAUUSD_M5_202601020105_202607312345.csv"
CONFIG_PATH = ROOT / "configs" / "strategies" / "rsiqui" / "final_m5_monthly_reset_source_b7313c09688e666c.json"
SYMBOL = "XAUUSD"
TIMEFRAME = "5m"
...
def main() -> None:
    ...
    if payload["strategy"] != "rsiqui-v3-final" or payload["symbol"] != SYMBOL or payload["timeframe"] != "M5":
        raise RuntimeError("This runner only accepts FINAL XAUUSD M5")
    ...
    config_payload = config.__dict__ | {
        "strategy": payload["strategy"], "symbol": SYMBOL, "timeframe": payload["timeframe"],
    ...
    strategy_name = "builtin_rsiqui-v3-final_gold-loose_both_vol0.05_risk25_reward25_spreadcap0.5_comm0.16_monthly_reset_source_b7313c09688e666c_M5"
    ...
    signal_counts = {"preview_long": 0, "preview_short": 0, "confirmed_long": 0, "confirmed_short": 0, "blocked_no_matching_close_confirm": 0, "blocked_blackout_gmt7": 0, "blocked_spread": 0, "entered_long": 0, "entered_short": 0, "overlapping_trades": 0}
    ...
    for index in range(warmup, len(data)):
        ...
        if active is not None:
            ...
            exit_price = None
            if active["side"] == "long":
                if row["low"] <= active["stop_price"]: exit_price = active["stop_price"]
                elif row["high"] >= active["take_profit_price"]: exit_price = active["take_profit_price"]
            else:
                if row["high"] >= active["stop_price"]: exit_price = active["stop_price"]
                elif row["low"] <= active["take_profit_price"]: exit_price = active["take_profit_price"]
