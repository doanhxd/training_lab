...
RAW_PATH = ROOT / "data" / "raw" / "mt5_exports" / "XAUUSD_M5_202607010000_202607311745.csv"
CONFIG_PATH = ROOT / "configs" / "strategies" / "rsiqui" / "final_m5_demo.json"
SYMBOL = "XAUUSD"
TIMEFRAME = "5m"
START_UTC = pd.Timestamp("2026-07-01T00:00:00Z")
NOW_UTC = pd.Timestamp("2026-07-31T23:59:59Z")
SPREAD_CAP = 0.4
...
def main() -> None:
    ...
    config_payload = config.__dict__ | {
        ...
        "max_spread_original_config": payload.get("max_spread"),
        "july_2026_spread_cap_override": SPREAD_CAP,
        "source_strategy_file": str(ROOT / "strategies" / "builtins" / "rsiqui" / "final.py"),
        "source_config_file": str(CONFIG_PATH),
        "blocked_entry_hours_gmt7_configured_not_applied_by_final_backtest": payload.get("blocked_entry_hours_gmt7", []),
    ...
    strategy_name = (
        ...
        f"original_unfiltered_M5_2026_07_fullcsv_1745_spreadcap{SPREAD_CAP:g}_spreadx{float(payload['spread_multiplier']):g}_"
    ...
    metadata = {
        ...
        "limitations": [
            "July-only full CSV through 2026-07-31 17:45 UTC with spread cap $0.40. final.py is the original/unfiltered RSIQUI implementation and uses numpy.gradient centered-gradient behavior; label as FORENSIC/original, not causal live-ready evidence.",
            "The configured GMT+7 blackout is copied from NEG config but not applied in final.py backtest because final.py original RsiquiV3Config has no blackout field.",
            "M5 OHLC replay cannot verify tick-level latency or within-bar fill order beyond the strategy engine collision rules. CSV spread points are normalized to price spread by csv_loader using inferred point size.",
        ],
    }
    artifacts = persist_run_artifacts(
        run_id=run_id,
        raw_spec={"strategy": payload["strategy"], "source": "final.py_original_unmodified", "raw_path": str(RAW_PATH), "period": "2026-07-01..2026-07-31 17:45", "requested_csv": "XAUUSD_M5_202607010000_202607311745.csv", "max_spread_override": SPREAD_CAP},
