...
CONFIG_PATH = ROOT / "configs" / "strategies" / "rsiqui" / "backtests" / "final_x_m5_backtest_20260101_present_sl15_tp15_vol003_eq5000_xauusdc.json"
SYMBOL = "XAUUSDc"
TIMEFRAME = "5m"
...
def main() -> None:
    ...
    if payload["strategy"] != "rsiqui-v3-final-x" or payload["symbol"] != SYMBOL or payload["timeframe"] != "M5":
        raise RuntimeError("This runner only accepts FINAL_X XAUUSDc M5")
    ...
    config_payload = config.__dict__ | {
        ...
        "lookahead_policy": payload.get("lookahead_policy", "causal_only_current_and_prior_RSI"),
        "causal_signal_engine": "backward_RSI_difference_no_future_rows",
    }
    config_hash = hashlib.sha256(json.dumps(config_payload, sort_keys=True, default=str).encode("utf-8")).hexdigest()[:16]
    strategy_name = "builtin_rsiqui-v3-final-x_gold-loose_both_XAUUSDc_M5_vol0.03_risk15_reward15_closeconfirm_20260101_present"
    ...
    metadata = {
        ...
        "limitations": [
            "causal root uses backward RSI difference and is prefix-invariant; centered-gradient look-ahead removed.",
