...
CONFIG_PATH = ROOT / "configs" / "strategies" / "rsiqui" / "backtests" / "final_trailing_20260101_present_sl30_tp30_vol003_milestone_trailing.json"
SYMBOL = "XAUUSD"
TIMEFRAME = "5m"
...
def main() -> None:
    ...
    strategy_name = "builtin_rsiqui-v3-final-trailing_gold-loose_both_vol0.03_risk30_reward30_milestone_trailing_blackout_gmt7_M5_20260101_present"
    ...
    for index in range(warmup, len(data)):
        ...
        if active is not None:
            ...
            if favorable_profit >= 25.0:
                locked_profit = 20.0 + max(0, int((favorable_profit - 25.0) / 1.0 + 1e-12)) * 1.0
                trailing_step = 1.0
                ...
                locked_profit = 15.0 + max(0, int((favorable_profit - 20.0) / 0.5 + 1e-12)) * 0.5
                trailing_step = 0.5
                ...
                locked_profit = 4.0 + max(0, int((favorable_profit - 6.0) / 0.5 + 1e-12)) * 0.5
                trailing_step = 0.5
