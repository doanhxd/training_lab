...
CONFIG_PATH = ROOT / "configs" / "strategies" / "ethan_xauusdc_m5_20260101_present_eq5000_vol003.json"
RAW_PATH = ROOT / "data" / "raw" / "mt5_exports" / "XAUUSDc_M5_202601012305_202608130240.csv"
...
def previous_completed_daily(m5: pd.DataFrame) -> pd.DataFrame:
    daily = m5.resample("1D", label="left", closed="left").agg({"open": "first", "high": "max", "low": "min", "close": "last"}).dropna()
    # A D1 bar becomes visible only at the following UTC daily boundary.
...
def main() -> None:
    ...
    daily = previous_completed_daily(m5)
    ...
    report = {"run_id": run_id, "strategy": payload["strategy"], "backtest_config": payload, "policy": policy, "data_coverage": quality(m5), "daily_reference_coverage": {"first_available_utc": str(daily.index.min()), "last_available_utc": str(daily.index.max()), "rows": int(len(daily))}, "metrics_summary": metrics(trades, float(payload["initial_equity"])), "signal_counts": {"completed_trades": int(len(trades)), "long_trades": int((trades["side"] == "long").sum()) if not trades.empty else 0, "short_trades": int((trades["side"] == "short").sum()) if not trades.empty else 0, "overlapping_trades": 0, "blocked_after_bankruptcy": blocked_after_bankruptcy}, "open_position_at_end": None, "artifacts": {"report": str(output / "report.json"), "trades": str(output / "trades.csv"), "equity": str(output / "equity_curve.csv")}}
