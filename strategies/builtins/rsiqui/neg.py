...
def is_gmt7_entry_blackout(entry_time: pd.Timestamp, blocked_hours_gmt7: tuple[int, ...]) -> bool:
    """Return whether the executable next-bar entry time is in a GMT+7 blackout hour."""
    timestamp = pd.Timestamp(entry_time)
    if timestamp.tzinfo is None:
        timestamp = timestamp.tz_localize("UTC")
    return timestamp.tz_convert("Asia/Ho_Chi_Minh").hour in blocked_hours_gmt7
...
def run_rsiqui_v3_backtest(features: pd.DataFrame, config: RsiquiV3Config) -> RsiquiV3Result:
    ...
    equity_curve: list[float] = []
    trades: list[TradeRecord] = []
    signal_counts = {"long_signal": 0, "short_signal": 0, "blocked_spread": 0, "blocked_gmt7": 0, "entered_long": 0, "entered_short": 0}
    ...
    for index in range(warmup, len(data) - 1):
        ...
        if active is not None:
            ...
            if exit_price is not None:
                ...
                # Charge adverse slippage independently at entry and exit.
                pnl -= (stressed_spread + 2.0 * config.slippage_per_side) * active["quantity"]
        ...
        if active is None:
            ...
            if side is not None:
                ...
                if is_gmt7_entry_blackout(next_row["timestamp"], config.blocked_entry_hours_gmt7):
                    signal_counts["blocked_gmt7"] += 1
